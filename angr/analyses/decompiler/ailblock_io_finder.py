...
class AILBlockIOFinder(AILBlockWalkerBase):
    ...
    def _handle_expr(
            self, expr_idx: int, expr: Expression, stmt_idx: int, stmt: Optional[Statement], block: Optional[Block], is_memory=False
    ) -> Any:
        try:
    ...
    def _handle_Load(self, expr_idx: int, expr: Load, stmt_idx: int, stmt: Statement, block: Optional[Block], is_memory=True):
        return self._handle_expr(0, expr.addr, stmt_idx, stmt, block, is_memory=True)

    def _handle_CallExpr(self, expr_idx: int, expr: Call, stmt_idx: int, stmt: Statement, block: Optional[Block], is_memory=False):
        args = set()
        if expr.args:
    ...
    def _handle_BinaryOp(self, expr_idx: int, expr: BinaryOp, stmt_idx: int, stmt: Statement, block: Optional[Block], is_memory=False):
        input_locs = set()
        self._add_or_update_set(
            input_locs,
            self._handle_expr(0, expr.operands[0], stmt_idx, stmt, block, is_memory=is_memory)
        )
        self._add_or_update_set(
            input_locs,
            self._handle_expr(1, expr.operands[1], stmt_idx, stmt, block, is_memory=is_memory)
        )

        return input_locs

    def _handle_UnaryOp(self, expr_idx: int, expr: UnaryOp, stmt_idx: int, stmt: Statement, block: Optional[Block], is_memory=False):
        return self._handle_expr(0, expr.operand, stmt_idx, stmt, block, is_memory=is_memory)

    def _handle_Convert(self, expr_idx: int, expr: Convert, stmt_idx: int, stmt: Statement, block: Optional[Block], is_memory=False):
        return self._handle_expr(expr_idx, expr.operand, stmt_idx, stmt, block, is_memory=is_memory)

    def _handle_ITE(self, expr_idx: int, expr: ITE, stmt_idx: int, stmt: Statement, block: Optional[Block], is_memory=False):
        input_locs = set()
        self._add_or_update_set(
            input_locs,
    ...
    def _handle_Tmp(self, expr_idx: int, expr: Tmp, stmt_idx: int, stmt: Statement, block: Optional[Block], is_memory=False):
        if self._as_atom:
            return None
        else:
            return expr

    def _handle_Register(self, expr_idx: int, expr: Register, stmt_idx: int, stmt: Statement, block: Optional[Block], is_memory=False):
        if self._as_atom:
    ...
    def _handle_StackBaseOffset(self, expr_idx: int, expr: StackBaseOffset, stmt_idx: int, stmt: Statement, block: Optional[Block], is_memory=False):
        if self._as_atom:
            return MemoryLocation(
                SpOffset(self.project.arch.bits, expr.size),
                expr.size * self.project.arch.byte_width
            )
        return expr
